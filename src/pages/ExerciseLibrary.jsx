...
function ImageViewerModal({ exercise, onClose }) {
  if (!exercise?.imageUrl) return null;

  return (
    <div className="el-image-viewer-overlay" onClick={onClose}>
      <button
        type="button"
      ...
      >
        ...
        />
        <div className="el-image-viewer-caption">{exercise.name}</div>
      </div>
    </div>
  );
}
...
function CreateExerciseModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (
        (existed || []).some(
          (x) =>
            ...
            newExerciseName.trim().toLowerCase()
        )
      ) {
      ...
      onCreated({
        exercise: ex,
        primarySubgroupId: newPrimarySubgroupId,
      });
  ...
  return (
    <div className="el-modal el-create-modal" onMouseDown={(e) => e.stopPropagation()}>
...
function PlanModal({
  ...
  const [planSetsCount, setPlanSetsCount] = useState("3");
  const [planSetRows, setPlanSetRows] = useState([
    { reps: "", weight: "" },
    { reps: "", weight: "" },
    { reps: "", weight: "" },
  ]);
  ...
  async function handleConfirm() {
    ...
    try {
      setSavingPlan(true);
      await onConfirm(buildSetTargetsPayload());
      onClose();
    } catch (err) {
      console.error(err);
      setSavingPlan(false);
  ...
  return (
    <div className="el-modal el-plan-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="el-plan-exercise-card">
          ...
          <div className="el-plan-exercise-info">
            ...
            <p className="el-plan-workout-name">
              Adding to <strong>{workoutName}</strong>
            </p>
          </div>
        </div>
          ...
          <div className="el-plan-sets-header">
            ...
            <div className="el-plan-sets-counter">
              <button
                ...
                onClick={() =>
                  onPlanSetsChange(String(Math.max(1, (Number(planSetsCount) || 0) - 1)))
                }
              ...
              <button
                ...
                className="el-plan-counter-btn"
                onClick={() =>
                  onPlanSetsChange(String((Number(planSetsCount) || 0) + 1))
                }
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M12 5v14M5 12h14" />
...
function ExerciseDetailsModal({
  ...
  return (
    <div className="el-modal" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className="el-modal-body">
        <section className="el-modal-section">
          <div className="el-modal-card el-modal-primary-card">
            ...
            <div className="el-modal-card-text">
              <div className="el-modal-card-label">Primary Target Muscle</div>
        ...
        <section className="el-modal-section">
          <div className="el-modal-card">
            <div className="el-modal-card-icon el-secondary-icon">⚡</div>
            <div className="el-modal-card-text">
        ...
        <section className="el-modal-section el-add-section">
            ...
            {workouts.map((w) => (
              <label
                ...
                className={`el-workout-item ${
                  selectedWorkoutId === w.id ? "el-workout-item--active" : ""
                }`}
              >
          ...
          <button
            ...
            onClick={() => onNext(selectedWorkoutId)}
          >
...
export default function ExerciseLibrary() {
  ...
  const filteredExercises = useMemo(() => {
    return exercises.filter((ex) => {
      if (filterGroup !== "all" && ex.primaryGroupKey !== filterGroup) return false;
      if (filterSubgroup !== "all" && ex.primarySubKey !== filterSubgroup) return false;
      if (filterEquipment !== "all" && ex.equipmentKey !== filterEquipment) return false;
      return true;
    });
  ...
  function openImageViewer(exercise) {
    let modalId = null;
  ...
  function openPlanModal(exercise, workoutId) {
  ...
  function openExerciseModal(exercise) {
    ...
    modalId = openModal(
      <ExerciseDetailsModal
        ...
        workouts={workouts}
        onClose={() => closeModal(modalId)}
        onOpenImage={() => openImageViewer(exercise)}
        onNext={(workoutId) => {
          closeModal(modalId);
          openPlanModal(exercise, workoutId);
        }}
      />,
      {
  ...
  const openCreateExerciseModal = useCallback(() => {
    ...
    modalId = openModal(
      <CreateExerciseModal
        ...
        onClose={() => closeModal(modalId)}
        onCreated={({ exercise, primarySubgroupId }) => {
          const primarySub = muscleSubgroups.find((sg) => sg.id === primarySubgroupId);
